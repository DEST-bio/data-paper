...
# scp aob2x@rivanna.hpc.virginia.edu:~/moments_o.genomalicious.neff.Rdata ~/.
  ...
  load("~/moments_o.genomalicious.neff.Rdata")
  o_wide[,divergence_time:=as.numeric(as.character(divergence_time_PoolSNP))]
  o.ag[,theta:=theta/83960116]
  oo <- melt(o.ag[,-"sampleId",with=F], id.vars=c("pair", "caller", "Pair_name", "status.x", "status.y"))
  ...
  dt <- oow[variable=="divergence_time" & PoolSNP > 100000]
  oow[variable=="theta" & PoolSNP > 0.006]
