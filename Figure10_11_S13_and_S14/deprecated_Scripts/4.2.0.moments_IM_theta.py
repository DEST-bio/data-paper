...
# Theta is parameterized for this run
...
PMmod=open('%s_output.IM.theta.txt' % Pair_name,'w')
PMmod.write(
...
#boundaries for nu1B, nu1F, nu2B, nu2F, Ts, m, theta1
upper_bound = [10, 10, 10, 10, 5, 50, 5e5]
lower_bound = [1e-5, 1e-5, 1e-5, 1e-5, 1e-5, 0, 3.75e5]
...
for i in range(int(iterations)): #iterations is imported from sys. argument #1
    ...
    popt=[np.random.uniform(lower_bound[0], upper_bound[0]), np.random.uniform(lower_bound[1], upper_bound[1]), np.random.uniform(lower_bound[2], upper_bound[2]), np.random.uniform(lower_bound[3], upper_bound[3]), np.random.uniform(lower_bound[4], upper_bound[4]), np.random.uniform(lower_bound[5], upper_bound[5]), np.random.normal(loc=theta_mean, scale=theta_sd, size=None)]
    ...
    popt=moments.Inference.optimize_log(popt, fs_folded, func_moments,
                                        ...
                                        verbose=False, maxiter=20, multinom=False,
                                        )

    #This number is 7. i.e., count parameters.
    params = len(["nu1B", "nu1F", "nu2B", "nu2F", "Ts", "m", "theta1"]) #for use in AIC calculation
    ...
    mig_pop2 = Mij*(nu2F*Nref) #number of individuals in pop2 from pop1
    ...
    PMmod=open('%s_output.IM.theta.txt' % Pair_name,'a')
