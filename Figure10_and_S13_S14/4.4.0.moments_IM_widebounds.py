...
PMmod=open('%s_output.IM.widebounds.txt' % Pair_name,'w')
PMmod.write(
...
#boundaries for nu1B, nu1F, nu2B, nu2F, Ts, m
upper_bound = [10, 10, 10, 10, 5, 50]
...
for i in range(int(iterations)): #iterations is imported from sys. argument #1
    ...
    popt=[np.random.uniform(lower_bound[0], upper_bound[0]), np.random.uniform(lower_bound[1], upper_bound[1]), np.random.uniform(lower_bound[2], upper_bound[2]), np.random.uniform(lower_bound[3], upper_bound[3]), np.random.uniform(lower_bound[4], upper_bound[4]), np.random.uniform(lower_bound[5], upper_bound[5])]
    ...
    #This number is 6. i.e., count parameters.
    params = len(["nu1B", "nu1F", "nu2B", "nu2F", "Ts", "m"]) #for use in AIC calculation
    ...
    PMmod=open('%s_output.IM.widebounds.txt' % Pair_name,'a')
